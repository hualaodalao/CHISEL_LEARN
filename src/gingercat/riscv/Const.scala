 ...
 */
case class IMM(instruction  : UInt) {
  // immediates
  ...
  def u = instruction(31 downto 12) ## 0.U(12.W)
  def j = instruction(31) ## instruction(19 downto 12) ## instruction(20) ## instruction(30 downto 21)
  def z = instruction(19 downto 15)

  // sign-extend immediates
  def i_sext = i.asSInt.pad(Riscv.XLEN)
  def h_sext = h.asSInt.pad(Riscv.XLEN)
  def s_sext = s.asSInt.pad(Riscv.XLEN)
  def b_sext = (b ## false.B).asSInt.pad(Riscv.XLEN)
  def j_sext = (j ## false.B).asSInt.pad(Riscv.XLEN)
}
...
object PrivilegeMode {
  ...
  def apply(isGuest: Bool, privilege: UInt): SInt = {
    ...
    // 高位：M 模式下恒为 0，否则取 isGuest
    val guestBit = Mux(privilege === PrivilegeMode.M.U, false.B, isGuest)
    (guestBit ## privilege).asSInt
  }
}
...
object CSR {
  val MCAUSE_ENUM = new {
    ...
    def isPageFault(code : UInt) : Bool = List(INSTRUCTION_PAGE_FAULT, LOAD_PAGE_FAULT, STORE_PAGE_FAULT).map(code === _.U).reduce(_ || _)
  }
