/** SpinalHDL spinal.lib.bus.regif.Doc.DocCHeader 的 Chisel 移植版。
  *
  * 与 SpinalHDL 的差异：
  *   - hexString 来自 RegIfDocHelper（等价 spinal.lib.BigIntRicher）；
  *   - mask 计算源码 `BigInt((1 << size) - 1)` 在 size ≥ 31 时 Int 溢出，
  *     此处改为 `(BigInt(1) << size) - 1`；
  *   - 其余（define/union/reuseDeclareFunc/reuseStruct 等）与源码一致。
  */
