/** SpinalHDL spinal.lib.bus.regif.Doc.DocHtml 的 Chisel 移植版。
  *
  * 与 SpinalHDL 的差异：
  *   - hexString 来自 RegIfDocHelper（等价 spinal.lib.BigIntRicher）；
  *   - getName/getFields/getAddr 等统一带括号调用；
  *   - 其余结构（TableTreeNode/HtmlSliceGrp/HtmlRegSliceBlock/两个分组器/
  *     TableTreeNodeImplicits）与源码一致。
  */
