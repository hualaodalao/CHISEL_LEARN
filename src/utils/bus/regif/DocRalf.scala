/** SpinalHDL spinal.lib.bus.regif.Doc.DocRalf 的 Chisel 移植版。
  *
  * 与 SpinalHDL 的差异：
  *   - hexString 来自 RegIfDocHelper（等价 spinal.lib.BigIntRicher）；
  *   - Field.section 源码为 downto Range（start=msb / end=lsb），本移植为
  *     `lo to hi`，故 rename 前缀用 `.max`/`${max}_${min}`、位偏移 `@` 用 `.min`
  *     保持输出语义一致。
  */
  ...
  implicit class FieldDescrExtend(fd: Field) {
    def toRalf(tab: String = ""): String = {
      def rename(name: String) = {
        val pre = if (fd.getWidth() == 1) s"${fd.getSection().max}" else s"${fd.getSection().max}_${fd.getSection().min}"
      ...
      s"""${tab}  field ${rename(fd.getName())} $forhdlpath @${fd.getSection().min} {
